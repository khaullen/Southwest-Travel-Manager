...
class SouthwestTravelManagerTests: XCTestCase {
    ...
    let testRealm = RLMRealm.inMemoryRealmWithIdentifier("test")
    ...
    override func tearDown() {
        testRealm.transactionWithBlock { () -> Void in
    ...
    func testCheckInReminders() {
        ...
        let dayAfterTomorrow = NSDate(timeIntervalSinceNow: 60 * 60 * 24 * 2)
        testFlight.outboundDepartureDate = dayAfterTomorrow
        testFlight.returnDepartureDate = dayAfterTomorrow.dateByAddingTimeInterval(60 * 60 * 24)
        
        testRealm.transactionWithBlock { () -> Void in
            self.testRealm.addObject(testFlight)
        }
